...
This is `base`, one repository of [Public Software](https://publicsoftware.dev): a spec-first cleanroom Rust suite built by people and coding agents together. Read this file before the README; it is the contract every agent works under here.

**Ring:** system · **Layers:** L4 · **Wave:** 1 · Init {{PURPOSE}} service manager, journal, util-linux/procps parity, disk encryption, accessibility bus, screen reader. Planned components: pubd-init · pubd-journal · utils · pub-crypt (LUKS2) · pub-a11y-bus · pub-reader.
...
- A new crate is `pub new <kind> <name>` with a kind from lib, app, service, plugin, spec; it lands in `crates/pub-base-<name>/` and gets a `[[component]]` entry in `CATALOG.toml`.

## Conventions

- Crates are named `pub-base-<name>`; a service is the binary `pubd-<name>`; WIT interfaces live under `public:`.
...
- The merge gates decide (`pub check`, CI, the agent review `suite / policy`, the maintainer team `public-software/maint-base`), not who or what wrote the change.
<!-- agents:end -->

## Repo notes

_Maintainers of `base` write here what the generated section cannot know: how the crates fit together, how to run the thing, which specifications matter, what not to touch. Everything above this heading is regenerated._
